...
def test_and_visualize_scenarios(
    ...
    figure_cluster_heatmap(cluster_summary_std, title_suffix="Standard Deviation (Original)")
    figure_cluster_heatmap(cluster_summary, title_suffix="Mean (Original)")

    selected_features = ['Anxiety', 'Depression', 'Hours per day', 'Music effects', 'While working']
    figure_numerical_distribution(df, 'Anxiety', cluster_labels)
    figure_numerical_distribution(df, 'Depression', cluster_labels)
    figure_cluster_correlation_heatmap(df, cluster_labels)
    ...
    for scenario_name, weighting_dict in scenarios.items():
        ...
        figure_cluster_heatmap(cluster_summary_std, title_suffix=f"Standard Deviation ({scenario_name})")
        figure_cluster_heatmap(cluster_summary, title_suffix=f"Mean ({scenario_name})")
        
        figure_numerical_distribution(df_weighted, 'Anxiety', cluster_labels_weighted)
        figure_numerical_distribution(df_weighted, 'Depression', cluster_labels_weighted)
